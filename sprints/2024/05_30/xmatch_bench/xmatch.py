...
def get_lsdb_catalogs(
    ...
    **kwargs,
    ...
    left = lsdb.read_hipscat(
        left_catalog_path,
        columns=left_catalog_columns,
    ).cone_search(
        ra=cone_ra_deg,
        dec=cone_dec_deg,
        radius_arcsec=cone_radius_deg * 3600,
    )
    right_margin = lsdb.read_hipscat(
        right_margin_path,
        columns=right_catalog_columns,
    )
    
    # https://github.com/astronomy-commons/lsdb/issues/335#issuecomment-2130344301
    right_margin._ddf.index = right_margin._ddf.index.astype(pd.ArrowDtype(pa.uint64()))

    right = lsdb.read_hipscat(
        right_catalog_path,
        columns=right_catalog_columns,
        margin_cache=right_margin,
    ).cone_search(
        ra=cone_ra_deg,
        dec=cone_dec_deg,
        radius_arcsec=cone_radius_deg * 3600,
...
def parse_args(cli_args):
    ...
    parser.add_argument(
        "--left-catalog-path",
        default="/ocean/projects/phy210048p/shared/hipscat/catalogs/ztf_axs/ztf_dr14",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--right-catalog-path",
        default="/ocean/projects/phy210048p/shared/hipscat/catalogs/gaia_dr3/gaia",
    )
    parser.add_argument(
        "--right-margin-path",
        default="/ocean/projects/phy210048p/shared/hipscat/catalogs/gaia_dr3/gaia_10arcs",
    )
    parser.add_argument(
...
def main(cli_args=None):
    ...
    with dask.distributed.Client(n_workers=args["dask_n_workers"]) as _client:
