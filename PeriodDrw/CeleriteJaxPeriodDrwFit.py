...
class CeleritePeriodDrwFit():

    def __init__(self, t=None, y=None, yerr=None, verbose=False):
    ...
    def build_gp(self, theta, t, y, yerr):
        ...
        self.kernel = CustomTerm(log_drw_amp=log_drw_amp, log_drw_scale=log_drw_scale,
                    log_per_amp=log_per_amp, log_per_scale=log_per_scale)

        gp = celerite.GP(self.kernel, mean=np.mean(y))
        gp.compute(t, yerr)
    ...
    def build_gp_drw(self, theta, t, y, yerr):
        ...
        self.kernel = CustomTerm_drw(log_drw_amp=log_drw_amp, log_drw_scale=log_drw_scale)
        gp = celerite.GP(self.kernel, mean=np.mean(y))
        gp.compute(t, yerr)
        gp.set_parameter_vector(theta)
        return gp
    ...
    def optimize(self, theta, t, y, yerr):
        ...
        """
        try:
            soln_celerite = sco.minimize(
                self.neg_log_likelihood, x0=theta, method="bfgs", args=(t, y, yerr))
            return soln_celerite.fun, soln_celerite.x, theta   
        except Exception as e:
            if self.verbose:
                warnings.warn("Failed to minimize.\n" + str(e))
            # Return a known "invalid" value to filter out
            # TODO wilsonbb fix these dimensions
            return np.array([np.inf]), np.full(len(theta), np.inf),  theta
    ...
    def optimize_map(self, t, y, yerr, n_init=100, use_pad=True, full=False):
        ...
        res = np.vstack(list(map(self.concatenate_arrays,
                                 many_init_res)))
        self.res = res
        ...
        if full:
            ...
            self.res_min = np.concatenate((np.array([min_log_likelihood]), self.res_min))
            return self.res_min
    ...
    def optimize_map_drw(self, t, y, yerr, n_init=100, use_pad=True, full=False):
        ...
        # Create a partially applied function
        # with fixed values of t, y, and yerr
        partial_optimize = partial(self.optimize_drw, t=t, y=y, yerr=yerr)
    ...
    def concatenate_arrays(self, array_tuple):
...
class JaxPeriodDrwFit(CeleritePeriodDrwFit):
    ...
    def __init__(self, t=None, y=None, yerr=None):
        ...
        # TODO: custom specification of padded values
        # TODO: experiment with gpu
        super(t=t, y=y, yerr=yerr)
        # TODO wilsonbb convert to jax arrays
    ...
    def neg_log_likelihood(self, theta, t, y, yerr):
        ...
        neg_log_likelihood : float
            Negative log-likelihood of the Gaussian Process model.
        """
        return super().neg_log_likelihood(theta, t, y, yerr)
        #gp = self.build_gp(theta, t, y, yerr)
        #return -gp.log_probability(y)
    ...
    def neg_log_likelihood_drw(self, theta, t, y, yerr):
        ...
        neg_log_likelihood : float
            Negative log-likelihood of the Gaussian Process model.
        """
        return super().neg_log_likelihood_drw(theta, t, y, yerr)
        gp = self.build_gp_drw(theta, t, y, yerr)
        return -gp.log_probability(y)
    ...
    def optimize_map(self, t, y, yerr, n_init=100, use_pad=True, full=False):
        ...
        super().optimize_map(t, y, yerr, n_init=n_init, use_pad=use_pad, full=full)
        ...
        res = np.vstack(list(map(concatenate_arrays,
                                 ...
                                 self.get_many_init_res(soln_res_map))))
        self.res = res
        res_min = self.find_best_res(res)
        self.res_min = res_min
    ...
    def get_many_init_res(self, soln_res_map):
        """"""
        return jax.device_get(many_init_res)
